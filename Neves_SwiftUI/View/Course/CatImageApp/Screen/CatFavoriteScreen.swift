    ...
    var body: some View {
        VStack {
            ...
            ScrollView {
                ...
                // 这样就不会不断地去【自动】加载更多了（使用LazyVStack就可以滑到底部时才去创建成功的ProgressView去加载更多）
                LazyVStack {
                    favoriteList
...
//        loadingState = await apiManager.getFavoritesAndAppend(page: page, limit: 5)
        ...
        // 而该方法第一句就是`loadingState = .loading`，导致原本的View直接被替换，顺带的`task`立马被取消执行，由于此时网络正在请求中，因此请求则会被取消（失败）
        // 🔨 解决方法：
